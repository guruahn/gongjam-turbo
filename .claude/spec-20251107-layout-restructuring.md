...
### 2. 그리드 적용 범위
- **적용**: 모든 라우트 (/, /hello, /home, 향후 /blog, /guest-book)
...
### 파일 생성 및 수정
- [ ] ProfileCard.vue 생성 및 packages/ui/src/index.ts export
- [ ] ProfileCard.spec.ts 작성
- [ ] portfolio.ts 데이터 파일 생성
- [ ] ShellLayout.vue 3열 그리드 추가
- [ ] ShellLayout.vue 푸터 링크 개선
- [ ] ShellLayout.spec.ts 업데이트
- [ ] hello-world/App.vue 리팩토링 (그리드 제거)
- [ ] hello-world/App.spec.ts 업데이트

### 빌드 및 테스트
- [ ] `pnpm turbo build` 성공 (모든 패키지/앱)
- [ ] `pnpm turbo type-check` 성공 (TypeScript 타입 체크)
- [ ] `pnpm turbo test` 성공 (모든 테스트 통과)

### 기능 동작
- [ ] `pnpm dev:all` 로컬 실행 정상
- [ ] Desktop (>= 1024px): 3열 그리드, 프로필 카드 전체 표시
- [ ] Mobile (< 1024px): 1열 스택, 프로필 카드 축소 (연락처 숨김)
- [ ] 프로필 카드 sticky positioning (top-20) 정상 동작
- [ ] 다크모드 토글 정상 동작
- [ ] 푸터 링크 hover 효과 동작
- [ ] 모든 라우트(/, /hello, /home)에서 동일한 레이아웃 표시

### 품질 기준
- [ ] 기존 다크모드 기능 유지
- [ ] 기존 네비게이션 기능 유지
- [ ] Module Federation 정상 동작
- [ ] 코드 중복 제거 (프로필 카드 1곳에서만 관리)
